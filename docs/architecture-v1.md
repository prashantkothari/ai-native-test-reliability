...
## 3e. Autonoma reference — architecture confirmation (BSL, read-only)

**Autonoma AI** ([github.com/Autonoma-AI/autonoma](https://github.com/Autonoma-AI/autonoma), TS pnpm monorepo, **BSL 1.1 — not OSS**) validates the package split we should target. **Do NOT copy code, do NOT fork** (BSL forbids competing use; and their runtime is Gemini/Prisma/S3/Sentry-heavy, opposite of our deterministic ethos). Read READMEs for shape, then write from our own spec — never read their source while writing ours.

| Autonoma package | Confirms our seam |
|---|---|
| `packages/agent-core` (dep-free: `ai`+`zod` only) | Our `selfheal-core` should be **dep-free** — Planner/Actor/Validator loop, tool taxonomy, step logging |
| `packages/engine` (shared driver interface) | Our `selfheal-runtime` needs a driver-plugin seam |
| `packages/engine-web` (Playwright adapter) | Matches our ADOPT-Playwright stance |
| `packages/engine-mobile` (Appium/WebDriverIO adapter) | **Direct precedent for our mobile expansion** — same pattern, same driver choice |
| `packages/visual-ai` (screenshot assert/choose/point/detect) | Our vision residue gate — separate package, sharp-native isolated |

**Net-new for us (blocker before mobile):** extract `selfheal-core` to a dep-free package. Today it carries deps that would leak into `engine-mobile`. This is a **P1 refactor**, not a free consequence of adding mobile.

---

## 3f. Recorded-step YAML format (Maestro-inspired)

Borrow **format grammar only** from Maestro ([mobile-dev-inc/maestro](https://github.com/mobile-dev-inc/maestro), Apache-2.0): flat command list, terse `- verb: value` form, `---` header/steps split. Semantics stay ours (11-signal descriptor, tier, fragility, verify-by-effect, intent).

```yaml
# flow: checkout.yaml
appId: shop.example.web            # or com.example.app for mobile
intent: "user completes a paid checkout with saved card"   # Clue-3 §1 intent — survives redesign
---
- goto: "/cart"
...
- tap:
    ...
    name: "Checkout"
    anchor:
      tier: testid                 # T0-T3: testid > stable-id > id-fragment > name > anchorless
      testid: "cart-checkout"
      container: { role: main, name: "Cart" }
      ordinal: null
    fragility: strong              # weak | ambiguous | strong (record-time flag → HITL route)
    verify:
      url_matches: "/checkout$"
...
- type:
    ...
    anchor:
      tier: name-only
      container: { role: form, name: "Contact" }
    text: "{{ user.email }}"
    verify: { value_equals: "{{ user.email }}" }

- tap:
    role: button
    name: "Pay"
    anchor: { tier: testid, testid: "pay-now" }
    verify:
      dom_appears: { role: heading, name: "Order confirmed" }
      network_settled: true        # in-flight==0, oldest-pending<500ms (K36 signal; web only until mobile hook)

- assert:
    visible: "Order confirmed"
```

**What is borrowed (gist):** flat step list; terse verb-inline happy path with nested-block escape hatch; `---` doc split.
**What is ours (not in Maestro):** `anchor.tier` (T0–T3), `fragility` flag, `verify:` as first-class per step, `intent:` at header, 11-signal descriptor, container/ordinal disambiguators.

---

## 3g. Mobile expansion — Appium, not a Maestro fork

**Decision: do NOT fork Maestro.** Do what Autonoma did: add `engine-mobile` on Appium/WebDriverIO, keep our self-heal core, share the YAML format across web+mobile.

**Why not fork:** (1) Maestro is Kotlin/JVM — language mismatch with our JS stack; (2) it's a full runner with its own flow-control, cloud, studio — 80% of code we don't want; (3) forking forces us to reject upstream changes that conflict with self-heal semantics; (4) the valuable bit is the **format**, which Apache-2.0 lets us copy freely.

**Plan:**
1. **P1 blocker:** extract `selfheal-core` to a dep-free package (per §3e).
2. **P2:** new `engine-mobile` package (Appium/WebDriverIO), mirrors `engine-web`'s driver interface.
3. **P2:** extend 11-signal descriptor to native accessibility (iOS `accessibilityIdentifier`, Android `resource-id`, `content-desc`) — most map cleanly onto existing tiers.
4. **P2:** extend FAILURE-TAXONOMY with two mobile-native residues: **KEYBOARD-COVERED**, **SYSTEM-DIALOG-INTRUSION**.
5. **P2 (authoring path):** **Maestro Studio → our-format transpiler.** Use Maestro Studio's record UX to author, transpile its YAML to ours at import. We get their authoring UX free; our runtime executes. (This is the mitigation for "rebuilding an authoring UX from scratch is a year of work.")

**Open gap:** `verify.network_settled` is CDP on web. On mobile it needs Appium's native network hooks or falls back to visual-stable + timeout. Flag as a per-driver capability, not an assumed runtime primitive.
